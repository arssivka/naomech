...
const ufvector4
Kinematics::getCOMc(const vector<double>& bodyAngles) {
...
Kinematics::slowCalculateChainCom(const ChainID id,
                                  const double angles[]) {
    ufmatrix4 fullTransform = ublas::identity_matrix<double>(4);
    ...
    for (int i = 0; i < numTransforms; i++) {
        ...
        // link
        const double* currentmDHParameters = MDH_PARAMS[id];

        //length L - movement along the X(i-1) axis
        if (currentmDHParameters[i * 4 + L] != 0) {
            const ufmatrix4 transX =
                    CoordFrame4D::translation4D(currentmDHParameters[i * 4 + L], 0.0, 0.0);
        ...
        if (currentmDHParameters[i * 4 + D] != 0) {
            const ufmatrix4 transZ =
                    CoordFrame4D::translation4D(0.0, 0.0, currentmDHParameters[i * 4 + D]);
            fullTransform = prod(fullTransform, transZ);
        }

        const double* curInertialPos = INERTIAL_POS[id];
        ...
        const double curMassProportion = curInertialPos[i * 4 + MASS_INDEX] / TOTAL_MASS;
