...
void Joints::initKeysMap(std::map<std::string, std::string> &map,
    ...
    while (first_it != keys.end() &&
           second_it != values.end()) {
        map.insert(
                make_pair<const std::string &, const std::string &>(*first_it,
                                                                    *second_it));
        ++first_it;
        ++second_it;
    }
...
bool Joints::set(const std::vector<std::string> &keys,
...
bool Joints::set(const std::vector<int> &keys,
                 const std::vector<float> &values, int time_offset) {
...
SensorData<float> Joints::getHardness(const std::vector<int> &keys) {
    return SensorData<float>(this->getValues(this->hardness_list, keys),
                             this->clock(0));
}
...
SensorData<float> Joints::getHardness(
        const std::vector<std::string> &keys) {
    return SensorData<float>(this->getValues(this->hardness_map, keys),
                             this->clock(0));
}
...
SensorData<float> Joints::getHardness() {
    return SensorData<float>(this->getValues(this->hardness_map, this->keys),
                             this->clock(0));
}
