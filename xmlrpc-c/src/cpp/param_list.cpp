...
namespace xmlrpc_c {
    ...
    paramList::getDouble(unsigned int const paramNumber,
        ...
        if (this->paramVector[paramNumber].type() != value::TYPE_DOUBLE)
            throw (fault(
                    "Parameter that is supposed to be floating point number "
                            "is not",
                    fault::CODE_TYPE));

        double const doublevalue(static_cast<double>(
                                         value_double(
                                                 this->paramVector[paramNumber])));
    ...
    paramList::getDatetime_sec(
        ...
        switch (constraint) {
            ...
            case TC_NO_FUTURE:
                if (timeValue > now)
                    throw (fault(
                            "Datetime parameter that is not supposed to be in "
    ...
    paramList::getString(unsigned int const paramNumber) const {
        ...
        return static_cast<string>(value_string(
                this->paramVector[paramNumber]));
    }
    ...
    paramList::getBytestring(unsigned int const paramNumber) const {
        ...
        if (paramP->type() != value::TYPE_BYTESTRING)
            throw (fault(
                    "Parameter that is supposed to be a byte string is not",
                    fault::CODE_TYPE));
    ...
    paramList::getI8(unsigned int const paramNumber,
        ...
        if (this->paramVector[paramNumber].type() != value::TYPE_I8)
            throw (fault(
                    "Parameter that is supposed to be 64-bit integer is not",
                    fault::CODE_TYPE));
        ...
        if (longlongvalue > maximum)
            throw (fault("64-bit integer parameter too high",
                         fault::CODE_TYPE));
