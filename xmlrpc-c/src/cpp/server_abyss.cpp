...
namespace xmlrpc_c {
    ...
    namespace {
        ...
        sigterm(int const signalClass) {
            ...
            cerr << "Signal of Class " << signalClass <<
            " received.  Exiting" << endl;
...
serverAbyss::constrOpt::OPTION_NAME(TYPE const& arg) { \
    ...
    DEFINE_OPTION_SETTER(portNumber, unsigned
            int);

    DEFINE_OPTION_SETTER(logFileName, string);

    DEFINE_OPTION_SETTER(keepaliveTimeout, unsigned
            int);

    DEFINE_OPTION_SETTER(keepaliveMaxConn, unsigned
            int);
    ...
    DEFINE_OPTION_SETTER(accessCtlMaxAge, unsigned
            int);
