...
-----------------------------------------------------------------------------*/
            ...
            if (!env.env_c.fault_occurred) {
                ...
                xmlrpc_serialize_call2(&env.env_c, callXmlMP,
                                       methodName.c_str(),
                                       paramArrayP, dialect);
...
-----------------------------------------------------------------------------*/
            ...
            if (!env.env_c.fault_occurred) {
                ...
                if (outcome.succeeded()) {
                    ...
                    *respXmlP = string(
                            XMLRPC_MEMBLOCK_CONTENTS(char, respXmlMP),
                            XMLRPC_MEMBLOCK_SIZE(char, respXmlMP));
                } else {
                    env_wrap cFault;

                    xmlrpc_env_set_fault(&cFault.env_c,
                                         outcome.getFault().getCode(),
                                         outcome.getFault().getDescription().c_str());

                    xmlrpc_serialize_fault(&env.env_c, respXmlMP,
                                           &cFault.env_c);
...
-----------------------------------------------------------------------------*/
            ...
            xmlrpc_parse_response2(&env.env_c, responseXml.c_str(),
                                   responseXml.size(),
