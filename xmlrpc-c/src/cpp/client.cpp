...
namespace xmlrpc_c {
    ...
    carriageParmPtr::carriageParmPtr(
            carriageParm *const carriageParmP) : autoObjectPtr(
            carriageParmP) { }
    ...
    carriageParm_http0::setUser(string const userid,
        ...
        xmlrpc_server_info_set_user(
                &env.env_c, this->c_serverInfoP, userid.c_str(),
                password.c_str());
    ...
    carriageParm_http0::disallowAuthDigest() {
        ...
        xmlrpc_server_info_disallow_auth_digest(&env.env_c,
                                                this->c_serverInfoP);
    ...
    carriageParm_http0::allowAuthNegotiate() {
        ...
        xmlrpc_server_info_allow_auth_negotiate(&env.env_c,
                                                this->c_serverInfoP);
    ...
    carriageParm_http0::setBasicAuth(string const username,
        ...
        xmlrpc_server_info_set_basic_auth(
                &env.env_c, this->c_serverInfoP, username.c_str(),
                password.c_str());
        throwIfError(env);
    }
...
-----------------------------------------------------------------------------*/
    ...
    clientXmlTransport::asyncComplete(
        ...
        try {
            if (transportEnv.fault_occurred) {
                xmlTranCtlP->xmlTranP->finishErr(
                        error(transportEnv.fault_string));
            } else {
    ...
    clientXmlTransportPtr::clientXmlTransportPtr(
            clientXmlTransport *const transportP) : autoObjectPtr(
            transportP) { }
    ...
    clientXmlTransport_http::setInterrupt(int *const interruptP) {
        ...
        if (this->c_transportOpsP->set_interrupt)
            this->c_transportOpsP->set_interrupt(this->c_transportP,
                                                 interruptP);
    }
    ...
    bool const haveLibwww(false);

    bool const haveWininet(false);
...
-----------------------------------------------------------------------------*/
    ...
    client_xml::call(carriageParm *const carriageParmP,
        ...
        xml::generateCall(methodName, paramList, this->implP->dialect,
                          &callXml);
...
-----------------------------------------------------------------------------*/
    ...
    rpc::getResult() const {
        ...
        switch (this->implP->state) {
            case rpc_impl::STATE_UNFINISHED:
                throw (error(
                        "Attempt to get result of RPC that is not finished."));
                break;
    ...
    rpc::getFault() const {
        ...
        switch (this->implP->state) {
            case rpc_impl::STATE_UNFINISHED:
                throw (error(
                        "Attempt to get fault from RPC that is not finished"));
                break;
            case rpc_impl::STATE_ERROR:
                throw (*this->implP->errorP);
                break;
            case rpc_impl::STATE_SUCCEEDED:
                throw (error(
                        "Attempt to get fault from an RPC that succeeded"));
                break;
            case rpc_impl::STATE_FAILED: {
                // All normal
            }
