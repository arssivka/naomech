 ...
 * special thanks to Orf or vosk for the KMat library
	 ...
	 *			-# HeadPitch
	 * .
 * \file libNAOKinematics.h
 
 This code uses a "fake" chain on the NAO robot but it equevelant to the real one.
 There are several fixes to trasnform the "real" chain to the "fake" one and they are located at the libNAOKinematics.cpp file and at the KinematicsDefines header.
...
public:
    enum Effectors {
        EFF_START = 0,
        EFF_CAMERA_BOT = KDeviceLists::CHAINS_SIZE,
        EFF_CAMERA_TOP,
        EFFECTOR_SIZE
    };
    enum Frames {
...
private:
     ...
     * */
    void prepareDerivatives(
            KDeviceLists::ChainsNames= KDeviceLists::CHAINS_SIZE);

    KVecDouble3 calculateCoMChain(Frames start, Frames BaseFrame, int startdown,
                                  float &mass);
...
public:
    ...
    bool setChain(KDeviceLists::ChainsNames ch,
    ...
    AngleContainer inverseHead(kmatTable targetPoint, bool withAngles,
                               bool topCamera);
...
private:
    ...
    AngleContainer jacobianInverseHead(kmatTable targetPoint,
                                       KDeviceLists::ChainsNames ch,
                                       bool topCamera);

    AngleContainer jacobianInverseHands(kmatTable targetPoint,
                                        KDeviceLists::ChainsNames ch);

    AngleContainer jacobianInverseLegs(kmatTable targetPoint,
                                       KDeviceLists::ChainsNames ch);


    static kmatTable getTransformation(const FKvars s) {
        kmatTable T;
        KMatTransf::makeTransformation(T, s.p(0), s.p(1), s.p(2), s.a(0),
                                       s.a(1), s.a(2));
        return T;
    }
