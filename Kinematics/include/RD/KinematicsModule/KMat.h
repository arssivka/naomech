...
namespace KMath {
    namespace KMat {
        ...
        public:
            MatrixIndexOutOfBoundsException(std::string m = "") : runtime_error(
                    m.append(
                            ": MatrixIndexOutOfBoundsException : attempted to access an invalid element")) { }
        };

        class SingularMatrixInvertionException : public std::runtime_error {
        public:
            SingularMatrixInvertionException(std::string m = "")
                    : runtime_error(m.append(
                    ": SingularMatrixInvertionException : attempted to invert a singular matrix")) { }
        };
        ...
        template<typename AT, unsigned AM, unsigned AN>
        class DataContainer {
        private:
            AT mem[AM][AN];
        ...
        template<typename T>
        class RefHandle<DataContainer<T, 1, 1> >
                : public LoopBackHandle<DataContainer<T, 1, 1> > {
        };

        template<typename T>
        class RefHandle<DataContainer<T, 2, 1> >
                : public LoopBackHandle<DataContainer<T, 2, 1> > {
        };

        template<typename T>
        class RefHandle<DataContainer<T, 3, 1> >
                : public LoopBackHandle<DataContainer<T, 3, 1> > {
        };
        ...
        class BaseMatrix : public RefHandle<DataContainer<T, M, N> > {
            template<template<typename, unsigned, unsigned> class AD, typename AT, unsigned AM, unsigned AN> friend
            class BaseMatrix;
        ...
        public:
            ...
            BaseMatrix(BaseMatrix<D, T, M, N> const &o)
                    : RefHandle<DataContainer<T, M, N> >(o) {
                //if(h!=NULL)
             ...
             **/
            D<T, M, N> &column_add(
                    BaseMatrix<D, T, M, 1> const &__RESTRICTED rop) {
                if (h == NULL || rop.h == NULL)
                    return static_cast< D<T, M, N> &> (*this);
             ...
             **/
            template<unsigned L>
            D<T, M, L>  operator*(BaseMatrix<D, T, N, L> const &rop) const {
                return slow_mult(rop);
            }

            template<unsigned L>
            D<T, M, L> slow_mult(
                    BaseMatrix<D, T, N, L> const &__RESTRICTED rop) const {
                D<T, M, L> res;
            ...
            D<T, M, N> &fast_mult(
                    BaseMatrix<D, T, N, N> const &rop) //in place mult!!!
            {
                if (h == NULL || rop.h == NULL)
                    return static_cast< D<T, M, N> &> (*this);
            ...
            D<T, M, N> abs() const {
                ...
                for (unsigned i = 0; i < M; i++) {
                    for (unsigned j = 0; j < N; j++) {
                        ngen(j, i) =
                                h->data(i, j) > 0 ? h->data(i, j) : -h->data(i,
                                                                             j);
                    }
                }
            ...
            D<T, M, N> const &prettyPrint() const {
                ...
                for (unsigned i = 0; i < M; i++) {
                    ...
                    for (unsigned j = 0; j < N; j++) {
                        ...
                        h->data(i, j) << ""; //setprecision(3)<<setw(6)<<
                    }
            ...
            COWRef<T, D<T, M, N> > operator()(unsigned i, unsigned j) {
                return COWRef<T, D<T, M, N> >(
                        static_cast< D<T, M, N> &> (*this), i, j);
            };

            template<typename AT>
            D<T, M, N> &operator=(BaseMatrix<D, AT, M, N> const &src) {
                for (unsigned i = 0; i < M; i++)
                    for (unsigned j = 0; j < N; j++)
        ...
        class GenMatrix : public BaseMatrix<GenMatrix, T, M, N> {
        public:
        ...
        class GenMatrix<T, S, S> : public BaseMatrix<GenMatrix, T, S, S> {
            //using BaseMatrix<KMat:::GenMatrix,T,S,S>::data;
            friend GenMatrix<T, S, S> &transpose_square_matrix<>(
                    GenMatrix<T, S, S> &athis);

            friend GenMatrix<T, S, S> &invert_square_matrix<>(
                    GenMatrix<T, S, S> &athis);
        ...
        template<typename T, unsigned S>
        class GenMatrix<T, S, 1> : public BaseMatrix<GenMatrix, T, S, 1> {
        ...
        public:
            ...
            COWRef<T, GenMatrix<T, S, 1> > operator()(unsigned i) {
                return COWRef<T, GenMatrix<T, S, 1> >(
                        static_cast< GenMatrix<T, S, 1> &> (*this), i, 0);
            };
        ...
        template<typename T>
        class GenMatrix<T, 1, 1> : public BaseMatrix<GenMatrix, T, 1, 1> {
            //Add single dimentionall acess operator
        public:
            ...
            explicit GenMatrix<T, 1, 1>(T ax)
                    : BaseMatrix<KMat::GenMatrix, T, 1, 1>() {
                this->get(0, 0) = ax;
            };

            COWRef<T, GenMatrix<T, 1, 1> > operator()(unsigned i) {
                return COWRef<T, GenMatrix<T, 1, 1> >(
                        static_cast< GenMatrix<T, 1, 1> &> (*this), i, 0);
            };
        ...
        template<typename T>
        class GenMatrix<T, 2, 1> : public BaseMatrix<GenMatrix, T, 2, 1> {
            //Add single dimentionall acess operator
        public:
            ...
            explicit GenMatrix<T, 2, 1>(T ax, T ay)
                    : BaseMatrix<KMat::GenMatrix, T, 2, 1>() {
                this->get(0, 0) = ax;
                this->get(1, 0) = ay;
            };
        ...
        template<typename T>
        class GenMatrix<T, 3, 1> : public BaseMatrix<GenMatrix, T, 3, 1> {
            //Add single dimentionall acess operator
        public:
            ...
            explicit GenMatrix<T, 3, 1>(T ax, T ay, T az)
                    : BaseMatrix<KMat::GenMatrix, T, 3, 1>() {
                this->get(0, 0) = ax;
                this->get(1, 0) = ay;
                this->get(2, 0) = az;
            };
        ...
        GenMatrix<A, S, S> &invert_square_matrix(GenMatrix<A, S, S> &athis) {
            ...
            for (unsigned k = 0; k < S; k++) {
                //Find pivot
                unsigned pvt = k;
                A pvtmx = (athis.read(k, k) >= 0 ? athis.read(k, k)
                                                 : -athis.read(k, k));
                for (unsigned l = k + 1; l < S; l++) {
                    if (pvtmx <
                        (athis(l, k) >= 0 ? athis(l, k) : -athis(l, k))) {
                        pvt = l;
        ...
        GenMatrix<A, 2, 2> &invert_square_matrix(GenMatrix<A, 2, 2> &athis) {
            ...
            A determ = athis.read(0, 0) * athis.read(1, 1) -
                       athis.read(0, 1) * athis.read(1, 0);

            //std::cout<<"Det:"<<determ<<std::endl;
            //std::cout<<"Eps:"<<std::numeric_limits<T>::epsilon()<<std::endl;
            if (determ > std::numeric_limits<A>::epsilon() &&
                determ != (A) 0) //can invert
            {
        ...
        GenMatrix<A, 3, 3> &invert_square_matrix(GenMatrix<A, 3, 3> &athis) {
            ...
            A m1 = athis.read(0, 0) * (athis.read(1, 1) * athis.read(2, 2) -
                                       athis.read(1, 2) * athis.read(2, 1));
            //Minor 2: based on 1,2
            A m2 = athis.read(0, 1) * (athis.read(1, 0) * athis.read(2, 2) -
                                       athis.read(2, 0) * athis.read(1, 2));
            //Minor 2: based on 1,3
            A m3 = athis.read(0, 2) * (athis.read(1, 0) * athis.read(2, 1) -
                                       athis.read(2, 0) * athis.read(1, 1));
            A determ = m1 - m2 + m3;
                ...
                determ != (A) 0) //can invert
            {
                ...
                athis.get(0, 0) = (t.read(1, 1) * t.read(2, 2) -
                                   t.read(1, 2) * t.read(2, 1)) / determ;
                athis.get(0, 1) = (t.read(0, 2) * t.read(2, 1) -
                                   t.read(0, 1) * t.read(2, 2)) / determ;
                athis.get(0, 2) = (t.read(0, 1) * t.read(1, 2) -
                                   t.read(0, 2) * t.read(1, 1)) / determ;
                athis.get(1, 0) = (t.read(1, 2) * t.read(2, 0) -
                                   t.read(1, 0) * t.read(2, 2)) / determ;
                athis.get(1, 1) = (t.read(0, 0) * t.read(2, 2) -
                                   t.read(0, 2) * t.read(2, 0)) / determ;
                athis.get(1, 2) = (t.read(0, 2) * t.read(1, 0) -
                                   t.read(0, 0) * t.read(1, 2)) / determ;
                athis.get(2, 0) = (t.read(1, 0) * t.read(2, 1) -
                                   t.read(1, 1) * t.read(2, 0)) / determ;
                athis.get(2, 1) = (t.read(0, 1) * t.read(2, 0) -
                                   t.read(0, 0) * t.read(2, 1)) / determ;
                athis.get(2, 2) = (t.read(0, 0) * t.read(1, 1) -
                                   t.read(0, 1) * t.read(1, 0)) / determ;
                return athis;
            }
        ...
        template<typename T, unsigned M, unsigned N>
        GenMatrix<T, M, N> &GenMatrix<T, M, N>::pseudo_invert(
                GenMatrix<T, M, N> &athis) {
        ...
        public:
            ...
            ATMatrix<T, S> &fast_mult(ATMatrix<T, S> const &rop) {
                ...
                if (AisIdentity == true) //Mult with id , just copy
                {
                    ...
                    AisIdentity = rop.AisIdentity;
                         ...
                         false) //Not id and not zero, just do the math
                {
                    A *= rop.A;
                    AisZero = rop.AisZero;
                }
            ...
            //Project a point !
            template<unsigned M>
            GenMatrix<T, S - 1, 1> transform(GenMatrix<T, S - 1, M> const &c,
                                             T hom = 1) const {
                ...
                if (BisZero ==
                    false) //do some more math for the translatonal part
                {
            ...
            ATMatrix<T, S> &fast_invert() {
                ...
                if (BisZero == false) {
                    if (AisIdentity == false &&
                        AisZero == false) //Just do the math
                    {
        ...
        public:
            ...
            template<typename T>
            static void makeRotation(ATMatrix<T, 3> &m, T theta) {
                m.identity();
                m.AisIdentity = false;
            ...
            template<typename T>
            static void makeRotation(GenMatrix<T, 2, 2> &m, T theta) {
                m(0, 0) = cos(theta);
                m(0, 1) = -sin(theta);
                m(1, 0) = sin(theta);
                m(1, 1) = cos(theta);
            ...
            template<typename T>
            static void makeShearX(ATMatrix<T, 3> &m, T factor) {
                m.identity();
                m.AisIdentity = false;
                m.A(0, 1) = factor;
            };
            ...
            template<typename T>
            static void makeShearY(ATMatrix<T, 3> &m, T factor) {
                m.identity();
                m.AisIdentity = false;
                m.A(1, 0) = factor;
            };
            ...
            template<typename T, unsigned S>
            static void makeTranslation(ATMatrix<T, S> &m,
                                        GenMatrix<T, S - 1, 1> const &t) {
                m.identity();
                m.AisIdentity = true;
                m.AisZero = false;
                m.BisZero = false;
            ...
            template<typename T, unsigned S>
            static void makeScale(ATMatrix<T, S> &m,
                                  GenMatrix<T, S - 1, 1> const &t) {
                m.identity();
                m.AisIdentity = false;
            ...
            template<typename T>
            static void makeRotationZ(ATMatrix<T, 4> &m, T theta) {
                m.identity();
                m.AisIdentity = false;
                m.A(0, 0) = cos(theta);
                m.A(0, 1) = -sin(theta);
            ...
            template<typename T>
            static void makeRotationY(ATMatrix<T, 4> &m, T theta) {
                m.identity();
                m.AisIdentity = false;
                m.A(0, 0) = cos(theta);
                m.A(0, 2) = sin(theta);
            ...
            template<typename T>
            static void makeRotationX(ATMatrix<T, 4> &m, T theta) {
                m.identity();
                m.AisIdentity = false;
                m.A(1, 1) = cos(theta);
                m.A(1, 2) = -sin(theta);
            ...
            template<typename T>
            static void makeTranslation(ATMatrix<T, 4> &Transl, T x, T y, T z) {
                Transl.identity();
            ...
            **/
            template<typename T>
            static void makeDHTransformation(ATMatrix<T, 4> &Transf, T a,
                                             T alpha, T d, T theta) {
                Transf.zero();
            ...
            template<typename T>
            static void makeTransformation(ATMatrix<T, 4> &Transf, T px, T py,
                                           T pz, T rx, T ry, T rz) {
                Transf.zero();
                Transf.AisZero = false;
                Transf.BisZero = false;
            ...
            template<typename T>
            static void makeRotationXYZ(ATMatrix<T, 4> &Rot, T xAngle, T yAngle,
                                        T zAngle) {
            ...
            template<typename T>
            static void makeRotationZYX(ATMatrix<T, 4> &Rot, T zAngle, T yAngle,
                                        T xAngle) {
                ATMatrix<T, 4> Rx, Ry, Rz;
                makeRotationX(Rx, xAngle);
                makeRotationY(Ry, yAngle);
            ...
            template<typename T>
            static GenMatrix<T, 4, 4> castToGenMatrix(ATMatrix<T, 4> At) {
                GenMatrix<T, 4, 4> Gen;
                Gen(0, 0) = At(0, 0);
                Gen(0, 1) = At(0, 1);
                Gen(0, 2) = At(0, 2);
            ...
            template<typename T>
            static void makeDHDerivative(GenMatrix<T, 4, 4> &Gen, T alpha,
                                         T theta) {
                Gen.zero();
                Gen(0, 0) = -sin(theta);
                Gen(0, 1) = -cos(theta);
