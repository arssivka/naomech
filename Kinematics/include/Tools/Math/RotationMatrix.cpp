...
RotationMatrix RotationMatrix::fromRotationX(const float angle) {
    const float c = cos(angle), s = sin(angle);
    return RotationMatrix(Vector3<>(1.f, 0.f, 0.f), Vector3<>(0.f, c, s),
                          Vector3<>(0.f, -s, c));
}

RotationMatrix RotationMatrix::fromRotationY(const float angle) {
    const float c = cos(angle), s = sin(angle);
    return RotationMatrix(Vector3<>(c, 0.f, -s), Vector3<>(0.f, 1.f, 0.f),
                          Vector3<>(s, 0.f, c));
}

RotationMatrix RotationMatrix::fromRotationZ(const float angle) {
    const float c = cos(angle), s = sin(angle);
    return RotationMatrix(Vector3<>(c, s, 0.f), Vector3<>(-s, c, 0.f),
                          Vector3<>(0.f, 0.f, 1.f));
}
