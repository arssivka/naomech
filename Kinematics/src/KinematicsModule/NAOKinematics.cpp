...
NAOKinematics::NAOKinematics() : T(FR_SIZE), Tjacobian(FR_SIZE),
                                 joints(FR_SIZE), coms(FR_SIZE),
                                 masses(FR_SIZE) {
    kmatTable t1, t2;
    ...
    KMatTransf::makeTranslation(T[FR_BASE_T + CHAIN_HEAD], 0.0, 0.0,
                                NeckOffsetZ);
    ...
    KMatTransf::makeRotationXYZ(t1, 0.0, 0.0, -PI_2);
    coms[R_ARM + ELBOW_ROLL] = KVecDouble3(RElbowRollX, RElbowRollY,
                                           RElbowRollZ);
    ...
    //--------LARM
    //End and Base effectors
    KMatTransf::makeTranslation(T[FR_BASE_T + CHAIN_L_ARM], 0.0,
                                (ShoulderOffsetY), ShoulderOffsetZ);
    ...
    //-----RLEG
    //End and Base effectors
    KMatTransf::makeTranslation(T[FR_BASE_T + CHAIN_R_LEG], 0.0, -HipOffsetY,
                                -HipOffsetZ);
    ...
    coms[R_LEG + HIP_YAW_PITCH] = KVecDouble3(RHipYawPitchX, RHipYawPitchY,
                                              RHipYawPitchZ);
    t1.fast_invert();
    ...
    KMatTransf::makeRotationXYZ(t1, PI_2, PI_2, 0.0);
    coms[R_LEG + KNEE_PITCH] = KVecDouble3(RKneePitchX, RKneePitchY,
                                           RKneePitchZ);
    coms[R_LEG + KNEE_PITCH] = t1.transform(coms[R_LEG + KNEE_PITCH]);

    KMatTransf::makeRotationXYZ(t1, PI_2, PI_2, 0.0);
    coms[R_LEG + ANKLE_PITCH] = KVecDouble3(RAnklePitchX, RAnklePitchY,
                                            RAnklePitchZ);
    coms[R_LEG + ANKLE_PITCH] = t1.transform(coms[R_LEG + ANKLE_PITCH]);

    KMatTransf::makeRotationXYZ(t1, PI, PI_2, 0.0);
    coms[R_LEG + ANKLE_ROLL] = KVecDouble3(RAnkleRollX, RAnkleRollY,
                                           RAnkleRollZ);
    ...
    //------LLEG
    //End and Base effectors
    KMatTransf::makeTranslation(T[FR_BASE_T + CHAIN_L_LEG], 0.0, HipOffsetY,
                                -HipOffsetZ);
    ...
    coms[L_LEG + HIP_YAW_PITCH] = KVecDouble3(RHipYawPitchX, -RHipYawPitchY,
                                              RHipYawPitchZ);
    t1.fast_invert();
    ...
    KMatTransf::makeRotationXYZ(t1, PI_2, PI_2, 0.0);
    coms[L_LEG + KNEE_PITCH] = KVecDouble3(RKneePitchX, -RKneePitchY,
                                           RKneePitchZ);
    coms[L_LEG + KNEE_PITCH] = t1.transform(coms[L_LEG + KNEE_PITCH]);

    KMatTransf::makeRotationXYZ(t1, PI_2, PI_2, 0.0);
    coms[L_LEG + ANKLE_PITCH] = KVecDouble3(RAnklePitchX, -RAnklePitchY,
                                            RAnklePitchZ);
    coms[L_LEG + ANKLE_PITCH] = t1.transform(coms[L_LEG + ANKLE_PITCH]);

    KMatTransf::makeRotationXYZ(t1, PI, PI_2, 0.0);
    coms[L_LEG + ANKLE_ROLL] = KVecDouble3(RAnkleRollX, -RAnkleRollY,
                                           RAnkleRollZ);
    ...
    KMatTransf::makeTranslation(T[FR_END_T + EFF_CAMERA_BOT], CameraBotomX, 0.0,
                                CameraBotomZ);
    KMatTransf::makeTranslation(T[FR_END_T + EFF_CAMERA_TOP], CameraTopX, 0.0,
                                CameraTopZ);
...
bool NAOKinematics::setChain(KDeviceLists::ChainsNames ch,
                             std::vector<AngleType> jointsset) {
    unsigned chsize;
    Frames frstart;
    switch (ch) {
        case CHAIN_HEAD:
...
void NAOKinematics::prepareForward(KDeviceLists::ChainsNames ch) {
    if (ch == CHAIN_L_ARM || ch == CHAINS_SIZE) {
        KMatTransf::makeDHTransformation(T[L_ARM + SHOULDER_PITCH], 0.0, -PI_2,
                                         0.0, (double) joints[L_ARM +
                                                              SHOULDER_PITCH]);
        KMatTransf::makeDHTransformation(T[L_ARM + SHOULDER_ROLL], 0.0, PI_2,
                                         0.0, (double) joints[L_ARM +
                                                              SHOULDER_ROLL] +
                                              PI_2);
        KMatTransf::makeDHTransformation(T[L_ARM + ELBOW_YAW], +ElbowOffsetY,
                                         PI_2, UpperArmLength,
                                         (double) joints[L_ARM + ELBOW_YAW]);
        KMatTransf::makeDHTransformation(T[L_ARM + ELBOW_ROLL], 0.0, -PI_2, 0.0,
                                         (double) joints[L_ARM + ELBOW_ROLL]);
        ...
        /*The fix*/
        KMatTransf::makeDHTransformation(T[L_ARM + WRIST_YAW], 0.0, PI_2, 0.0,
                                         (double) joints[L_ARM + WRIST_YAW]);
    }

    if (ch == CHAIN_R_ARM || ch == CHAINS_SIZE) {
        KMatTransf::makeDHTransformation(T[R_ARM + SHOULDER_PITCH], 0.0, -PI_2,
                                         0.0, (double) joints[R_ARM +
                                                              SHOULDER_PITCH]);
        KMatTransf::makeDHTransformation(T[R_ARM + SHOULDER_ROLL], 0.0, PI_2,
                                         0.0, (double) joints[R_ARM +
                                                              SHOULDER_ROLL] +
                                              PI_2);
        KMatTransf::makeDHTransformation(T[R_ARM + ELBOW_YAW], -ElbowOffsetY,
                                         PI_2, UpperArmLength,
                                         (double) joints[R_ARM + ELBOW_YAW]);
        KMatTransf::makeDHTransformation(T[R_ARM + ELBOW_ROLL], 0.0, -PI_2, 0.0,
                                         (double) joints[R_ARM + ELBOW_ROLL]);
        /*Correct one BUT THE FIX does not change anythging*/
        //KMatTransf::makeDHTransformation(T[R_ARM+WRIST_YAW], 0.0, PI_2, LowerArmLength, (double)joints[R_ARM+WRIST_YAW]);
        /*The fix*/
        KMatTransf::makeDHTransformation(T[R_ARM + WRIST_YAW], 0.0, PI_2, 0.0,
                                         (double) joints[R_ARM + WRIST_YAW]);
    }

    if (ch == CHAIN_L_LEG || ch == CHAINS_SIZE) {
        KMatTransf::makeDHTransformation(T[L_LEG + HIP_YAW_PITCH], 0.0,
                                         -3 * PI_2 / 2, 0.0,
                                         (double) joints[L_LEG +
                                                         HIP_YAW_PITCH] - PI_2);
        KMatTransf::makeDHTransformation(T[L_LEG + HIP_ROLL], 0.0, -PI_2, 0.0,
                                         (double) joints[L_LEG + HIP_ROLL] +
                                         PI_2 / 2);
        KMatTransf::makeDHTransformation(T[L_LEG + HIP_PITCH], 0.0, PI_2, 0.0,
                                         (double) joints[L_LEG + HIP_PITCH]);
        KMatTransf::makeDHTransformation(T[L_LEG + KNEE_PITCH], -ThighLength,
                                         0.0, 0.0,
                                         (double) joints[L_LEG + KNEE_PITCH]);
        KMatTransf::makeDHTransformation(T[L_LEG + ANKLE_PITCH], -TibiaLength,
                                         0.0, 0.0,
                                         (double) joints[L_LEG + ANKLE_PITCH]);
        KMatTransf::makeDHTransformation(T[L_LEG + ANKLE_ROLL], 0.0, -PI_2, 0.0,
                                         (double) joints[L_LEG + ANKLE_ROLL]);
    }

    if (ch == CHAIN_R_LEG || ch == CHAINS_SIZE) {
        KMatTransf::makeDHTransformation(T[R_LEG + HIP_YAW_PITCH], 0.0,
                                         -PI_2 / 2, 0.0, (double) joints[R_LEG +
                                                                         HIP_YAW_PITCH] -
                                                         PI_2);
        KMatTransf::makeDHTransformation(T[R_LEG + HIP_ROLL], 0.0, -PI_2, 0.0,
                                         (double) joints[R_LEG + HIP_ROLL] -
                                         PI_2 / 2);
        KMatTransf::makeDHTransformation(T[R_LEG + HIP_PITCH], 0.0, PI_2, 0.0,
                                         (double) joints[R_LEG + HIP_PITCH]);
        KMatTransf::makeDHTransformation(T[R_LEG + KNEE_PITCH], -ThighLength,
                                         0.0, 0.0,
                                         (double) joints[R_LEG + KNEE_PITCH]);
        KMatTransf::makeDHTransformation(T[R_LEG + ANKLE_PITCH], -TibiaLength,
                                         0.0, 0.0,
                                         (double) joints[R_LEG + ANKLE_PITCH]);
        KMatTransf::makeDHTransformation(T[R_LEG + ANKLE_ROLL], 0.0, -PI_2, 0.0,
                                         (double) joints[R_LEG + ANKLE_ROLL]);
    }

    if (ch == CHAIN_HEAD || ch == CHAINS_SIZE) {
        KMatTransf::makeDHTransformation(T[HEAD + YAW], 0.0, 0.0, 0.0,
                                         (double) joints[HEAD + YAW]);
        KMatTransf::makeDHTransformation(T[HEAD + PITCH], 0.0, -PI_2, 0.0,
                                         (double) joints[HEAD + PITCH] - PI_2);
    }
}
...
NAOKinematics::kmatTable NAOKinematics::getForwardFromTo(Effectors start,
                                                         Effectors stop) {
...
KVecDouble3 NAOKinematics::calculateCoMChain(Frames start, Frames BaseFrame,
                                             int startdown, float &mass) {
    KVecDouble3 pp, cp;
    float pm = 0;
    pp.zero();
    cp.zero();
...
KVecDouble3 NAOKinematics::calculateCenterOfMass() {
    ...
    p = calculateCoMChain((Frames) HEAD, (Frames) (FR_BASE_T + CHAIN_HEAD),
                          HEAD_SIZE, m);
    sumc = p;
    summass = m;

    p = calculateCoMChain((Frames) L_ARM, (Frames) (FR_BASE_T + CHAIN_L_ARM),
                          ARM_SIZE, m);
    sumc = sumc * summass + p * m;
    summass += m;
    sumc.scalar_mult(1. / summass);

    p = calculateCoMChain((Frames) R_ARM, (Frames) (FR_BASE_T + CHAIN_R_ARM),
                          ARM_SIZE, m);
    sumc = sumc * summass + p * m;
    summass += m;
    sumc.scalar_mult(1. / summass);

    p = calculateCoMChain((Frames) L_LEG, (Frames) (FR_BASE_T + CHAIN_L_LEG),
                          LEG_SIZE, m);
    sumc = sumc * summass + p * m;
    summass += m;
    sumc.scalar_mult(1. / summass);

    p = calculateCoMChain((Frames) R_LEG, (Frames) (FR_BASE_T + CHAIN_R_LEG),
                          LEG_SIZE, m);
    sumc = sumc * summass + p * m;
    summass += m;
    sumc.scalar_mult(1. / summass);
...
NAOKinematics::AngleContainer NAOKinematics::inverseHead(const FKvars s,
                                                         bool withAngles,
                                                         bool topCamera) {
    return inverseHead(getTransformation(s), withAngles, topCamera);
}

NAOKinematics::AngleContainer NAOKinematics::inverseHead(kmatTable targetPoint,
                                                         bool withAngles,
                                                         bool topCamera) {
    kmatTable T;
    kmatTable outputT;
    ...
    if (withAngles) {
        ...
        for (int j = 0; j < 2; j++) {
            if (j == 0 && (tempTheta2 - psi > HeadYawHigh ||
                           tempTheta2 - psi < HeadYawLow))
                continue;
            else if (j == 1 && (posOrNegPI - tempTheta2 - psi > HeadYawHigh ||
                                posOrNegPI - tempTheta2 - psi < HeadYawLow))
                continue;
            else if (j == 1)
            ...
            if (topCamera) {
                ...
                downt1 =
                        CameraBotomZ * cos(theta2) - CameraBotomX * sin(theta2);
            }
            ...
            for (int i = 0; i < 2; i++) {
                if (i == 0 && (theta1 > HeadPitchHigh || theta1 < HeadPitchLow))
                    continue;
                         ...
                         (-theta1 > HeadPitchHigh || -theta1 < HeadPitchLow))
                    continue;
                else if (i == 0)
                    theta1 = theta1;
                else
            ...
            outputT = getForwardEffector(
                    topCamera ? EFF_CAMERA_TOP : EFF_CAMERA_BOT);
...
NAOKinematics::AngleContainer NAOKinematics::inverseLeftHand(
        kmatTable targetPoint) {
    ...
    for (int i = 0; i < 2; i++) {
        if (i == 0 && (theta2temp - PI_2 > LShoulderRollHigh ||
                       theta2temp - PI_2 < LShoulderRollLow)) {
            continue;
        } else if (i == 1 && (-theta2temp - PI_2 > LShoulderRollHigh ||
                              -theta2temp - PI_2 < LShoulderRollLow)) {
            continue;
        } else if (i == 1) {
        ...
        KMatTransf::makeDHTransformation(T3, +ElbowOffsetY, PI_2,
                                         UpperArmLength, theta3);
        try {
            T3.fast_invert();
        }
        catch (KMath::KMat::SingularMatrixInvertionException d) {
...
std::vector<std::vector<float> > NAOKinematics::inverseRightHand(
        const FKvars s) {
...
std::vector<std::vector<float> > NAOKinematics::inverseLeftLeg(
        kmatTable targetPoint) {
    std::vector<std::vector<float> > returnResult;
    ...
    double theta4 = PI - acos((pow(side1, 2) + pow(side2, 2) - distancesqrd) /
                              (2 * side1 * side2));
    ...
    for (int itter = 0; itter < 2; itter++) {
        ...
        double up =
                TtempTheta5(1, 3) * (TibiaLength + ThighLength * cos(theta4)) +
                ThighLength * TtempTheta5(0, 3) * sin(theta4);
        double down = pow(ThighLength, 2) * pow(sin(theta4), 2) +
                      pow(TibiaLength + ThighLength * cos(theta4), 2);
        ...
        for (int i = 0; i < 2; i++) {
            if (i == 0 && (theta5 > LAnklePitchHigh || theta5 < LAnklePitchLow))
                continue;
            else if (i == 1 && (posOrNegPIt5 - theta5 > LAnklePitchHigh ||
                                posOrNegPIt5 - theta5 < LAnklePitchLow))
                continue;
            else if (i == 1)
                theta5 = posOrNegPIt5 - theta5;

            KMatTransf::makeDHTransformation(T5i, -TibiaLength, 0.0, 0.0,
                                             theta5);
            Ttemp = T4i;
            Ttemp *= T5i;
            ...
            for (int l = 0; l < 2; l++) {
                if (l == 0 && (temptheta2 - PI_2 / 2 > LHipRollHigh ||
                               temptheta2 - PI_2 / 2 < LHipRollLow))
                    continue;
                else if (l == 1 && (-temptheta2 - PI_2 / 2 > LHipRollHigh ||
                                    -temptheta2 - PI_2 / 2 < LHipRollLow))
                    continue;
                else if (l == 0)
                ...
                if (theta3 != theta3 &&
                    Ttemp2(1, 1) / sin(theta2 + PI_2 / 2) < 0)
                    theta3 = -PI_2;
                ...
                for (int k = 0; k < 2; k++) {
                    if (k == 0 &&
                        (theta3 > LHipPitchHigh || theta3 < LHipPitchLow))
                        continue;
                    else if (k == 1 && (posOrNegPIt3 - theta3 > LHipPitchHigh ||
                                        posOrNegPIt3 - theta3 < LHipPitchLow))
                        continue;
                    else if (k == 1)
                        theta3 = posOrNegPIt3 - theta3;

                    float temptheta1 = acos(
                            Ttemp2(0, 2) / sin(theta2 + PI_2 / 2));
                    ...
                    for (int p = 0; p < 2; p++) {
                        ...
                        if (p == 0 && (temptheta1 + PI_2 > LHipYawPitchHigh ||
                                       -temptheta1 + PI_2 < LHipYawPitchLow))
                            continue;
                        else if (p == 1 &&
                                 (-temptheta1 + PI_2 > LHipYawPitchHigh ||
                                  -temptheta1 + PI_2 < LHipYawPitchLow))
                            continue;
                        else if (p == 0)
                        ...
                        kmatTable test = getForwardEffector(
                                (Effectors) CHAIN_L_LEG);
...
std::vector<std::vector<float> > NAOKinematics::inverseRightLeg(
        const FKvars s) {
